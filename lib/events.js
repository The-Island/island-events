...
Events.prototype.send = function (channel, topic, data) {
  console.log('send:', [channel, topic, JSON.stringify(data)].join(' '));
...
Events.prototype.subscribe = function (subscriber, subscribee, meta, cb) {
  ...
  // For subscriptions that are created out of a user mentioning another:
  var mentioner = meta.mentioner;
  delete meta.mentioner;
  ...
  self.db.Subscriptions.read(props, function (err, sub) {
    ...
    Step(
      ...
      function (err, stat, notes) {
        ...
        if (sub.meta.style === 'watch') {
          if (self.sock) {
            self.send('watch', 'watch.removed', {id: sub._id.toString()});
          }
        } else {
...
Events.prototype.publish = function (channel, topic, params, cb) {
  ...
  console.log(params)
  console.log(_.deepClone(params))
  ...
  if (self.sock) {
    var data = iutil.client(_.deepClone(params.data));
    if (params.data.public !== false) {
      ...
      var author_id = params.data.author ? params.data.author._id:
          ...
          console.log('adsfvdfvdfvdfvdffdfdfdfdfdfdfdfddf')
  ...
  Step(
    function () {
      if (params.event._id) {
        self.db.Events.update({_id: params.event._id},
            {$set: params.event.$set}, _.bind(function (err) {
          if (err) return this(err);
          self.db.Events.read({_id: params.event._id}, this);
        }, this));
      } else {
        params.event.date = params.data.date || params.data.created;
        self.db.Events.create(params.event, this);
      }
    },
    function (err, event) {
      if (err) return cb(err);
      ...
      Step(
          ...
          _.each(subs, function (s) {
            self.db.inflate(s, {subscribee: {collection: s.meta.type, '*': 1}},
              ...
              hasAccess(self.db, s.subscriber, s.subscribee, function (err, allow) {
                console.log('weradfvadfvondafv', err)
                if (err) return _this(err);
                if (!allow) {
                  s._reject = true;
                }
        ...
        function (err, subs) {
          ...
          Step(
            function () {
              if (_.contains(['watch', 'follow', 'request', 'accept', 'mention'],
                  data.action_type)) {
                return this();
              }
...
Events.prototype.inflate = function(e, query, cb) {
  ...
  function __post(o, p, cb) {
    Step(
      function () {
        ...
        hasAccess(self.db, requestor, o[p], this);
      },
      function (err, allow) {
        if (err) return this(err);
        if (!allow) {
    ...
    Step(
      ...
      function (err) {
        ...
        _.each(o[p].actions, function (a) {
          _.each(a.ticks, function () { ++tickCnt; });
        });
        if (tickCnt === 0) {
          return this();
        }
      ...
      function (err) {
        ...
        if (tickCnt === 0) {
          ...
          if (o[p].weather && o[p].weather.daily &&
              o[p].weather.daily.data) {
            o[p].weather = {daily: o[p].weather.daily.data[0]};
          }
        }
        cb();
      }
    ...
    Step(
      ...
      function (err, allow) {
        ...
        self.db.inflate(t, {author: profiles.member, ascent: profiles.ascent,
            crag: profiles.crag, session: profiles.session}, this.parallel());
  ...
  function __crag(o, p, cb) {
    Step(
      function () {
        self.db.inflate(o[p], {author: profiles.member, parent: profiles.crag},
            this.parallel());
        self.db.fill(o[p], 'Hangtens', 'parent_id', this.parallel());
      }, cb
    );
      ...
      function (err) {
        ...
        switch (parent_type) {
          ...
          case 'tick':
            ...
            break;
          case 'crag':
            __crag(o, 'target', this);
            break;
          case 'ascent':
            __ascent(o, 'target', this);
            break;
        }
      },
      cb
    ...
    function () {
      switch (e.action_type) {
        ...
        case 'tick':
          ...
          break;
        case 'crag':
          __crag(e, 'action', this);
          break;
        case 'ascent':
          __ascent(e, 'action', this);
          break;
        case 'hangten':
          __hangten(e, this);
          break;
